package org.example.WorkingWithDBs.JDBCChallenges;
...
//video 367
/*
- change the structure of the order_details table, adding a new
  column called quantity, a number
        - then proceed with adding data
- use data from Orders.csv to add orders to the storefront database
- use PreparedStatements to insert each order, and its related items
- batch up the line items for each order, but batch only line
  items for a single order
- use a transaction for each individual order rolling back the
  order insert if something fails
...
//            addOrders(conn, orders);
    ...
    private static void addOrders(Connection conn, PreparedStatement psOrder,
    ...
    private static void addOrders(Connection conn, List<Order> orders) {
        ...
        try (PreparedStatement psOrder = conn.prepareStatement(insertOrder,
            ...
            orders.forEach((o) -> {
                try {
                    addOrders(conn, psOrder, psDetail, o);
                } catch (SQLException e) {
                    System.out.printf("%d (%s) %s%n", e.getErrorCode(),
